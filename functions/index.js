...
export const createShipment = functions.https.onRequest(async (req, res) => {
  ...
  try {
    const { orderId, recipient, payment_mode, total_amount, pickup_address_id, shipment_details, items } = req.body;
...
export const registerPickup = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error in Registering Order Pickup:",
      error.message
    );
    res.status(500).send({
...
export const trackingHistory = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error Fetching Order Tracking History:",
      error.message
    );
    res.status(500).send({
...
export const reattemptOrder = functions.https.onRequest(
  async (req, res) => {
    if (req.method !== "POST") {
      return res.status(405).send({ error: "Method Not Allowed" });
    }
    ...
    try {
      const { apiorderid, action, reattempt_date, contact_name, complete_address, landmark, mobilenumber, remarks } = req.body;
