	...
	return (
		<div className="px-20 pt-10 w-full flex flex-col gap-10">
			<div className="w-full flex gap-14 h-full">
				<div className="flex gap-8 relative w-1/2">
				...
				<div className="w-1/2 flex flex-col justify-between">
					<div className="flex flex-col justify-between">
						<div className="flex justify-between items-center"><h2 className="xl:text-2xl 2xl:text-4xl font-semibold text-gray-800 leading-3">{productData?.name}</h2><img className="h-7" src={hovered ? shareIconActive : shareIcon} onMouseOver={() => setHovered(true)} onMouseOut={() => setHovered(false)} alt="" /></div>
						<p className="text-sm text-gray-500 flex border-b-2 border-[rgb(8,43,61)] pb-1 2xl:text-base">
							<span className="opacity-50 mr-7 font-semibold tracking-tight">APPAREL & FASHION</span>
							{
								[1, 2, 3, 4, 5].map((_, index) => (
									<i key={index} className="fi fi-ss-star text-yellow-400 relative top-[1px] "></i>
								))
							}
							<span className="ml-1 font-medium">74 Reviews</span>
						</p>
						<div className="flex gap-2 items-center xl:mt-3 2xl:mt-5">
						...
						<div className="flex items-center gap-1 rounded-tl-full rounded-br-full bg-[rgb(8,43,61)] w-[145px] 2xl:w-[170px] mt-3 2xl:mt-5 px-3 2xl:px-5 py-[3px] 2xl:py-[4px]"><img src={lxsLogo} alt="" className="h-5" /> <span className="text-sm 2xl:text-base text-white font-medium">LXS Certified</span></div>
						...
						<div className="xl:mt-3 2xl:mt-5 flex flex-col xl:gap-3 2xl:gap-5">
							<div className="flex flex-col justify-between h-full">
								...
								<div className="flex space-x-3">
									{sizes.map((size) => (
										<button
											key={size}
											className={`xl:h-12 2xl:h-14 xl:w-12 2xl:w-14 border rounded-full text-lg font-semibold transition ${selectedSize === size ? "bg-[rgb(8,43,61)] text-white" : "border-[rgb(8,43,61)] lg:hover:bg-gray-200"
												}`}
							...
							<div className="flex flex-col justify-between gap-1">
								<p className="font-semibold 2xl:text-lg">Quantity:</p>
								<div className="flex items-center justify-center rounded-full overflow-hidden w-32 xl:h-11 2xl:h-12 border border-[rgb(8,43,61)]">
									<button
										className="h-full border-r border-[rgb(8,43,61)] rounded-l-md text-lg w-[34%] active:bg-gray-200 outline-none"
										onClick={() => setQty((q) => Math.max(1, q - 1))}
									>
										-
									</button>
									<span className="h-full w-[32%] text-lg flex items-center justify-center">{qty}</span>
									<button
										className="h-full w-[34%] border-l border-[rgb(8,43,61)] rounded-r-md text-lg active:bg-gray-200 outline-none"
						...
						<div className="xl:mt-3 2xl:mt-10">
							...
							<p className="text-sm 2xl:text-base leading-4 2xl:leading-6 font-medium">
								...
								Brand: <Link className="text-orange-500 lg:hover:underline">LXS</Link> <br />
								Seller: <Link className="text-orange-500 lg:hover:underline">LXS (OPC) PRIVATE LIMITED</Link> <br />
							</p>
						</div>
					</div>
