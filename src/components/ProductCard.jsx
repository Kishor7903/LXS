...
function ProductCard({ item }) {
    ...
    return (
        <div className="w-[100%] rounded-2xl overflow-hidden p-[6px] md:p-2 xl:p-3 cursor-pointer duration-200 border-slate-300 border-[1px] bg-white relative shadow-md">
            ...
            <div className="relative h-[100px]">
                ...
                <div className="flex justify-between items-center w-full bg-transparent relative bottom-1">
                    <p className='text-[16px] font-bold'>₹{item.salePrice} <s className='text-slate-400 mr-1 text-[10px] md:text-xs lg:text-[12px] xl:text-[13px] font-semibold'>₹{item.price}</s><span className='text-[rgb(253,84,120)] text-[10px] md:text-xs lg-text-[14px] xl:text-[12px] hidden lg:inline-block'>(-{`${Math.floor(((item.price - item.salePrice) * 100) / item.price)}`}%)</span></p>
                </div>
                <div className="flex justify-between items-center w-full">

                    <button onClick={() => user ? cart?.some((p) => p.item_id === item.id) ? navigate("/checkout/cart") : setIsOpen(true) : toast("Login Required.")} className={`flex items-center justify-center text-base font-semibold py-1 rounded-xl w-[82%] h-9  tracking-tight ${user ? cart?.some((p) => p.item_id === item.id) ? "bg-gradient-to-r from-[rgb(248,181,44)] to-[rgb(253,84,120)] text-white lg:hover:scale-[1.04] duration-200 lg:active:scale-[0.97]" : "bg-white border-2 border-[rgb(8,43,61)] lg:hover:bg-[rgb(8,43,61)] lg:hover:text-white" : "bg-white border-2 border-[rgb(8,43,61)] lg:hover:bg-[rgb(8,43,61)] lg:hover:text-white"}`}><i className="fi fi-sr-cart-shopping-fast relative top-0.5 text-lg mr-2"></i>{user ? cart?.some((p) => p.item_id === item.id) ? "Go to": "Add to" : "Add to"} Basket</button>
