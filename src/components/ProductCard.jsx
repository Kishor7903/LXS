        ...
        <div className="w-[100%] rounded-xl md:rounded-[10px] overflow-hidden p-[6px] md:p-2 xl:p-3 cursor-pointer duration-200 border-slate-300 border-[1px] bg-white relative shadow-md">
            ...
            <div className="relative h-[100px]">
                ...
                <div className="flex justify-between items-center w-full">
                    ...
                    <button onClick={(e) => cart?.some((p) => p.item_id === item.id) ? navigate("/checkout/cart") : addCart(e, item.id)} className={`flex items-center justify-center text-base font-semibold py-1 rounded-full w-[82%] lg:hover:scale-[1.03] duration-150 lg:active:scale-[0.97] lg:hover:shadow-[0px_0px_8px_-3px_rgb(8,43,61)] tracking-tight ${cart?.some((p) => p.item_id === item.id) ? "bg-[rgb(8,43,61)] text-white" : "bg-gradient-to-r from-[rgb(248,181,44)] to-[rgb(240,85,120)] text-white"}`}><i className="fi fi-sr-cart-shopping-fast relative top-0.5 text-lg mr-2"></i>{cart?.some((p) => p.item_id === item.id) ? "Go to": "Add to"} Basket</button>
